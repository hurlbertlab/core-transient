...
#The following script demonstrates the cutoffs of 80% 
#This script explores cutoffs of: 
#4/5 & 1/5 (original occ-scale-processing and creation of bbs_allscales.csv data)
#The supplemental cutoff script can be found in allscales_coefs50.R, original in core_scale_analyses.R

###################################################################################################################
####Original and working derivation 4/5 & 1/5 cutoffs####
...
fifty_allyears = read.csv("intermed/fifty_allyears.csv", header = TRUE) #using updated version, 80 stop data, 07/12
...
write.csv(bbs_below80, "intermed/bbs_below80.csv", row.names = FALSE)

#NOW I can average for unique scale-route combo (currently duplicates based on segments)

bbs_below_avgs80 = bbs_below80 %>% 
...
write.csv(bbs_below_avgs80, "intermed/bbs_below_avgs80.csv", row.names = FALSE)
...
bbs_above80 = as.data.frame(output)
#Calc area for above route scale
#bbs_above$area = bbs_above_v2$numrtes*80*(pi*(0.4^2)) #number of routes * fifty stops * area in sq km of a stop 
write.csv(bbs_above80, "intermed/bbs_above80.csv", row.names = FALSE)
#updated 05/03 with everything immediately preceding 

####Merging across scales####
bbs_above80 = read.csv("intermed/bbs_above80.csv", header = TRUE)
bbs_below80 = read.csv("intermed/bbs_below_avgs80.csv", header = TRUE)

#adding maxRadius column to bbs_below w/NA's + renaming and rearranging columns accordingly, creating area cols
bbs_below2 = bbs_below80 %>% 
  mutate(maxdist = c("NA")) %>%
...
write.csv(bbs_allscales, "intermed/bbs_allscales80.csv", row.names = FALSE) #saved 05/03
...
bbs_allscales = read.csv("intermed/bbs_allscales80.csv", header = TRUE)
...
write.csv(core_coefs80, "intermed/core_coefs80.csv", row.names = FALSE) 
