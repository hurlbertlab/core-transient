...
occ_counts = function(countData, countColumns, scale) {
  ...
  abun.summ = bbssub %>% #abundance
    ...
    dplyr::summarize(totalN = sum(groupCount)) %>%
    group_by(stateroute) %>%
    dplyr::summarize(aveN = mean(totalN))
    
  occ.summ = bbsu %>% #occupancy
    count(stateroute, AOU) %>%
    mutate(occ = n/15, scale = scale, subrouteID = countColumns[1]) %>%
    group_by(stateroute) %>%
    dplyr::summarize(meanOcc = mean(occ), 
              pctCore = sum(occ > 2/3)/length(occ),
              pctTrans = sum(occ <= 1/3)/length(occ),
              pctTrans25 = sum(occ <= 1/4)/length(occ), pctTrans10 = sum(occ <= 0.1)/length(occ)) %>%
...
for (scale in bscales) {
  ...
  for (g in 1:numGroups) {
    ...
    output = rbind(output, temp, sum(fifty_bestAous)) 
  }
}
...
bbs_allscales3 = filter(bbs_allscales, focalrte %in% bbs_allscales2$focalrte) %>% data.frame()
#write.csv(bbs_allscales3, "C:/git/core-transient/scripts/R-scripts/scale_analysis/bbs_allscales.csv", row.names = FALSE)
#updated 07/03
