...
################################################################################*
#
# Dataset name: Long-term stem inventory data from tropical rain forest plots in Australia: Trees >10 cm dbh
...
#--! PROVIDE INFO !--#
  'CSIRO_PermanentPlots_TreeMeasurementData.csv') 
...
#--! PROVIDE INFO !--#
unusedFieldNames = c('stemNumber', 'family', 'genus', 'taxonAuth',
                     'ALA_APNI_Taxon_LSID', 'ALA_APNI_Taxon_URI',
                     'wetTropicsEndemic', 'coordinates_x_metres', 'coordinates_y_metres',
                     'dbh_centimetres', 'comment', 'establishmentHeight_metres',
                     'height_metres', 'status')

# Using the status field to eliminate all dead trees
dataset1 = dataset[dataset$status != 'Dead', !names(dataset) %in% unusedFieldNames]
...
site_grain_names = c("site","epNumber")
