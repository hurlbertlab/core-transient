################################################################################*
#  DATASET 147: New Zealand Marine Life
#
# Metadata can be found at http://gcmd.gsfc.nasa.gov/KeywordSearch/Metadata.do?Portal=GCMD&MetadataType=0&MetadataView=Full&KeywordPath=&EntryId=[GCMD]OBIS_NZMBIS
...
site_grain_names = c("lat","long")

# We will now create the site field with these codes concatenated if there
# are multiple grain fields. Otherwise, site will just be the single grain field.
num_grains = length(site_grain_names)

# Site entries look like "Pacific_sitenumber_lat_long"
# There are too few years per site if just looking at site number
# Only the lat/longs are useful, so they will be extracted
...
# Dataset has less than minNTime years per site, but they are lat/longs and can be rounded later.
...
dataFormattingTable[,'Notes_siteFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_siteFormat',  # Fill value below in quotes
                                 ...
                                 'site field looks like "Pacific_sitenumber_lat_long", only the lat/longs were extracted. New site field looks like "lat_long"')
...
# Now explore the listed species themselves, again. A good trick here to finding problematic

levels(dataset5$species)

table(dataset5$species)

# These genus-only entries had entries of many species below it. They were deleted because they could be any of the possible species 

bad_sp = c('Bugula_sp.','Caberea_sp.','Catenicella_sp.','Chiastosella_sp.','Crepidacantha_sp.')
...
# The Conescharellina genus has one species listed along with the genus-only entry, so the species entries are combined into genus-only

typo_name = c('Conescharellina_angulopora',
              'Crisia_?acropora')

good_name = c('Conescharellina_sp.',
              'Crisia_acropora')

levels(dataset6$species) = c(levels(dataset6$species),'Crisia_acropora')
...
dataFormattingTable[,'Notes_spFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_spFormat',    # Fill value below in quotes
                                 ...
                                 'several species removed because the genus-only entry had many possible species underneath. One species was combined with the corresponding genus-only entry because it was the only possible listed species under the genus. One typo (an insertion of a "?") was also fixed.')
...
### NO ACCEPTABLE SITES, PROCESSING WILL NOT CONTINUE ###
