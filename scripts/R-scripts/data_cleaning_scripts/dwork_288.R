...
################################################################################*
#
# Dataset name: Fish population on selected watersheds at Konza Prairie
# Dataset source (link): http://www.konza.ksu.edu/knz/pages/data/KnzEntity.aspx?id=CFP012
...
# We can see that each fish species is in its own column, and we would like to
# reshape this data using the tidyr library
library(tidyr)
dataset1a = gather(dataset, key = species, value = abundance, CAMANO:MOXMIC)
...
#--! PROVIDE INFO !--#
unusedFieldNames = c('dataset', 'rectype')

dataset1 = dataset1a[, !names(dataset1a) %in% unusedFieldNames]
...
# Site names have extraneous spaces at the end
as.character(unique(dataset2$watershed))
# "N4D  " "MID  " "N1B  " "NT   " "NIB  " "K2A  "
...
# What is the spatial grain of the finest sampling scale? Based on 
# Bertrand, Gido & Guy (2006, Trans KS Acad Sci), mean pool width was 3.45 m
...
#--! PROVIDE INFO !--#
  'Unclear whether habitat is nested in replicate or vice versa, but these are nested within watershed.')
...
#--! PROVIDE INFO !--#
# a watershed is characterized by multiple electrofishing samples, each of which
# is too small to represent an entire assemblage. Plus, the sampling design is 
# slightly unclear as to whether replicates are conducted within 'habitat' or
# across 'habitat's.
