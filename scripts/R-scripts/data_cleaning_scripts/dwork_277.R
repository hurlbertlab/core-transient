...
################################################################################*
#
# Dataset name: North Temperate Lakes LTER: Zooplankton - Trout Lake Area
# Dataset source (link): https://lter.limnology.wisc.edu/data/filter/5733
...
#--! PROVIDE INFO !--#
  'north_temperate_lakes_lter__zooplankton_-_trout_lake.csv') 
...
#--! PROVIDE INFO !--#
unusedFieldNames = c('species_code', 'individuals_measured', 'avg_length', 'year4')
...
#--! PROVIDE INFO !--#
    'Stripped out hours and minutes')
...
# First we need to fix a typo ("Tr") that is apparent when checking the lakeid's:
...
# "2-meter high, 45L Schindler-Patalas trap with 53um mesh net and cup"
...
#--! PROVIDE INFO !--#
  'The site field is a concatenation of lakeid & station, although there only seems to be one station per lakeid')
...
#--! PROVIDE INFO !--#                                 
'number per liter summed over sex and stage and volumetrically integrated over the water column')
...
#--! PROVIDE INFO !--#
bad_sp = c('ASCOMORPHA', 'DAPHNIA', 'EUCYCLOPS', 'GASTROPUS', 'KELLICOTTIA',
           'KERATELLA', 'LECANE', 'MONOSTYLA', 'NOTHOLCA', 'POLYARTHRA',
           'SKISTODIAPTOMUS ', 'TRICHOCERCA', 'UNIDENTIFIED', 'UNKNOWN',
           'UNKNOWN EGG-SHAPED ROTIFER', 'UNKNOWN ROTIFER', '')
...
#--! PROVIDE INFO !--#                                 
  'When unids within a genus were <50% of the genus, they were removed; otherwise all spp in genus lumped with genus')
...
#--! PROVIDE INFO !--#
# there seems to be only one station per lake anyway.
