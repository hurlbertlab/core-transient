...
################################################################################*
#
# Dataset name: Long-term stem inventory data from tropical rain forest plots in Australia: understory plants
# Dataset source (link): http://esapubs.org/archive/ecol/E095/209/metadata.php
...
#--! PROVIDE INFO !--#
  'CSIRO_PermanentPlots_UnderstoryData.csv') 
...
#--! PROVIDE INFO !--#
unusedFieldNames = c('family', 'genus', 'taxonAuth',
                     'ALA_APNI_Taxon_LSID', 'ALA_APNI_TAXON_URI')

# Using the status field to eliminate all dead trees
...
# The dateformat of these data is "%b-%y", but for some reason R is not 
# recognizing this. So, I am pasting "01-" in front of all dates, and then
# telling R the format is "%d-%b-%y" which then does work.
dataset1$date = paste("01", dataset1$date, sep = "-")
...
#--! PROVIDE INFO !--#
    'Original dates include only month and year; had to add arbitrary day (1) to read it.')
...
#--! PROVIDE INFO !--#
site_grain_names = c("epNumber", "subPlot")
...
# THESE DATA ARE ONLY AVAILABLE AT THE TIME OF ESTABLISHMENT, NOT A TIME SERIES,
...
# Noticed one typo ("eP4" instead of "ep4"; there are only supposed to be 20 sites total)
site[site == 'eP4'] = 'ep4'
...
#--! PROVIDE INFO !--#
  'Each site is a 0.5 ha plot; there are coordinates for each stem, so scale evaluation could be possible, but not in existing cleaning script')
...
#--! PROVIDE INFO !--#                                 
              'Each record is the presence of 1 individual')
...
#--! PROVIDE INFO !--#
typo_name = c('Austrosteenisia blackii var. blackii',
              'Euroschinus falcatus var. falcatus')

#--! PROVIDE INFO !--#
good_name = c('Austrosteenisia blackii',
...
#--! PROVIDE INFO !--#                                 
  'Taxonomy seem solid, just a few blank species records removed, and two varieties collapsed to parents')
