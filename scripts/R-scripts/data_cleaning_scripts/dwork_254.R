################################################################################*
# Dataset 254, Portal Mammals
#
# Data available from Ecodata Retriever (PortalMammals_main), metadata at
...
# Note that for this dataset, because there is a species with code 'NA' (Neotoma albigula),
# and because NA is not used to indicate "Not Available" or "Not Applicable" anywhere
# according to the metadata, we will read this data in by turning off the NA string,
# which would otherwise convert all N. albigula records to <NA>.
dataset = read.csv(paste('data/raw_datasets/dataset_', datasetID, '.csv', sep = ''), na.strings = "")
...
unusedFieldNames = c('period','note1','stake','sex','age','reprod','testes','vagina','pregnant',
                 'nipples','lactation','hfl','wgt','tag','note2','ltag','note3','prevrt',
...
# What is the name of the field(s) that has information on sampling date?
...
dataFormattingTable[,'Notes_timeFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_timeFormat',  # Fill value in below
                                 ...
                                 'year, month, and day fields combined into single date field')
...
dataset3 = dataset3[,-which(names(dataset3) == 'plot')]
...
dataFormattingTable[,'Notes_siteFormat'] = 
  ...
  'data can be analyzed at the scale of the entire "site" or at the "plot"')
...
dataset3$count = 1 #since every row indicates a single individual
countfield = 'count'
...
dataset5 = subset(dataset4, !is.na(count))
...
dataFormattingTable[,'Notes_countFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_countFormat', # Fill value below in quotes
                                 ...
                                 'Each row represents the trapping of a single individual, so count=1')
...
# All non-rodent species, and rodent species only identified to Genus will be excluded.
# This information can be found in the PortalMammals_species table available from
# Ecodata Retriever

bad_sp = c('AB', 'AH', 'AS', 'CB', 'CM', 'CQ', 'CS', 'CT', 'CU', 'CV', 'EO', 'GS',
           'PC', 'PG', 'PU', 'SA', 'SB', 'SC', 'SS', 'ST', 'SU', 'UL', 'UP', 'US',
           'UR', 'XX', 'ZL', 'ZM', 'DX', 'NX', 'OX', 'PX', 'RX', 'SX')
...
# If there are no typos, set typo_name = c().
...
dataFormattingTable[,'Notes_spFormat'] = 
  ...
  'All non-rodents that were not identified to species were removed based on the species table associated with this dataset.')
...
# ...there is really only a single rodent community at Portal. Plots are immediately
# adjacent to one another. (Could evaluate exclosures vs controls though...)
