################################################################################*
#  DATASET 99: Australian Marine Life
#
...
# Let's change the name of the "Species" column to "species":
...
# Site entries look like "Courageous_survey_Cour_031_(1978)_station_no_10_extracted_from_CMAR_Data_Warehouse_on_12_Oct_2005_42.5_41_40_45" 

# Extracting only useful info: station number and lat/long

newsite = c()

for (i in 1:length(site)){
if (word(site[i],1,sep="_") == ' Sprightly' | word(site[i],1,sep="_") == ' Soela') {
  
  newsite[i] = word(site[i],7,sep="_")
  
} else {
  
  newsite[i] = word(site[i],8,sep="_")
}
...
# Dataset has mostly more than minNTime years per site 
...
dataFormattingTable[,'Notes_siteFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_siteFormat',  # Fill value below in quotes
                                 ...
                                 'site field had lots of unusable data (like the name of the ship used) separated by underscores. Only the station number was extracted.')
...
dataFormattingTable[,'Notes_spFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_spFormat',    # Fill value below in quotes
                                 ...
                                 'no bad species or typos to correct.')
...
sGrain = 'stationnumber'

# This is a reasonable choice of spatial grain because there is little helpful info in the metadata, but the stations are spaced around the entire coast of australia, so they should be spread far enough that they each reperesent fish community size.
