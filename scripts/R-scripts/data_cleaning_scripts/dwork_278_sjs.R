...
################################################################################*
#
# Dataset name:North Temperate Lakes LTER: Phytoplankton - Trout Lake Area

# Dataset source (link):https://lter.limnology.wisc.edu/dataset/north-temperate-lakes-lter-phytoplankton-trout-lake-area-1984-current
...
#--! PROVIDE INFO !--#
  'north_temperate_lakes_lter__phytoplankton_-_trout_lake_area.csv') 
...
unusedFieldNames = c('division', 'year4','cells_per_nu', 'depth_range', 'nu_per_ml',
                     'biovolume_conc','biomass_conc', 'relative_total_biovolume',
                     'gald', 'genus')

###to standardize depth range, subsetting to depth range = 0 only
# No good metadata on meaning of "depth_range", but most of data
# is for depth_range==0, although only from 2 lakes.
#    lakeid depth_range Freq
#4      CR           0  623
#5      SP           0  801
#8      AL HP 0-bottom   91
#9      BM HP 0-bottom   82
#11     CR HP 0-bottom  255
#12     SP HP 0-bottom  284
#14     TR HP 0-bottom  293
#17     CB    Int 0-2m   65
#20     TB    Int 0-2m   58
dataset1 = dataset[dataset$depth_range == "0", !names(dataset) %in% unusedFieldNames]
...
#--! PROVIDE INFO !--#
bad_sp = c('Dinobryon', 'Oscillatoria', 'Peridinium', 'Staurastrum')
...
#--! PROVIDE INFO !--#
typo_name = c('Anabaena circinalis',
              'Closterium moniliferum',
              'Cosmarium tenue',
              'Kephyrion gracile',#found only one record of this name on Google
              'Mallomonas akrokomas',
             'Mallomonas caudata') 

#--! PROVIDE INFO !--#
good_name = c('Anabaena',
             'Closterium',
             'Cosmarium',
             'Kephyrion gracilis',
             'Mallomonas',
...
#--! PROVIDE INFO !--#
# there is only 1 station per lake, so this is the only appropriate scale to analyze the data
