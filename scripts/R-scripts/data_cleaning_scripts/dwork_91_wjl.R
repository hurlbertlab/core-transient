################################################################################*
#  DATASET 91: Baltic Seabirds
#
#  Metadata can be found at http://seamap.env.duke.edu/dataset/358
...
# Site entries all have 'BalticSea_RigaGulf' followed by lat/long coordinates. Only the lat/longs are useful, so everything else will be removed. 

site = paste(word(site, c(3), sep="_"),word(site, c(4), sep="_"), sep="_")
...
# Each lat/long site does not have enough years per site, but they will be rounded later.
...
dataFormattingTable[,'Notes_siteFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_siteFormat',  # Fill value below in quotes
                                 ...
                                 'info in site entries reduced from "BalticSea_RigaGulf_lat_long" to just "lat_long" ')
...
# If there are entries that only specify the genus while there are others that specify the species in addition to that same genus, they need to be regrouped in order to avoid vague specificity. For example, if there are entries of 'Cygnus', 'Cygnus_columbianus', and 'Cygnus_cygnus', 'Cygnus' could refer to either species, but the observer could not identify it. This causes ambiguity in the data, and must be fixed by either 1. deleting the genus-only entry altogether, or 2. renaming the genus-species entries to just the genus-only entry, combining them. 
# This decision can be fairly subjective, but generally if less than 25% of the entries are genus-only, then they can be deleted. If more than 25% of the entries for that genus are only specified to the genus, then the genus-species entries should be renamed to be genus-only. 

# Removing Larus entry because there were very few entries of just 'Larus' and thousands of entries of Larus identified to the species

bad_sp = c('LARUS')

dataset6 = dataset5[!dataset5$species %in% bad_sp,]

# It may be useful to count the nmber of times each name occurs, as misspellings or typos will likely
# only show up one time.

table(dataset6$species)

# No typos, changing names of some genus-species entries to genus-only entries.

typo_name = toupper(c('Anas_crecca',
              'Anas_platyrhynchos',
              'Cygnus_columbianus',
              'Cygnus_cygnus',
              'Cygnus_olor',
...
dataFormattingTable[,'Notes_spFormat'] = 
  dataFormattingTableFieldUpdate(datasetID, 'Notes_spFormat',    # Fill value below in quotes
                                 ...
                                 'There were many instances where a genus would have some entries identified to the species and some not. The Larus entries were removed because there were so few of them compared to entries identified to the species. All entries of the genus Anas, Cygnus, and Gavia were renamed to genus-only entries because there were many entries that were not identified to the species')
...
# This is a reasonable choice of spatial grain because it yields 4 sites that are an appropriate distance away to represent bird communities. 
