...
################################################################################*
#
# Dataset name: Fourteen years of mapped, permanent quadrats in a northern mixed prairie, USA
# Dataset source (link): http://esapubs.org/archive/ecol/E092/143/metadata.htm
...
#--! PROVIDE INFO !--#
  'Quad was appended to a constant site number (1) so that data can be aggregated across all quads.')
...
#--! PROVIDE INFO !--#                                 
              'Number of individuals of each species per quad and year')
...
#--! PROVIDE INFO !--#
bad_sp = c('Atriplex sp.', 'Chamaesyce sp.', 'Chenopodium sp.', 'Cryptantha sp.',
           'Euphorbia sp.', 'Plantago sp.', 'Polygonum sp.', 'unknown', 
           'unknown annual', 'unknown forb')
...
#--! PROVIDE INFO !--#
typo_name = c('Androsace sp.', # assign to only other Androsace sp in dataset
              'Collinsia sp.', # assign to only other Collinsia sp in dataset
              'Collomia sp.',  # assign to only other Collomia sp in dataset
              'Draba sp.',     # assign to only other Draba sp in dataset
              'Hedeoma sp.',   # assign to only other Hedeoma sp
              'Helianthus sp.',# assign to only other Helianthus
              'Lactuca sp.',   # assign to only other Lactuca
              'Lappula sp.',   # assign to only other Lappula
              'Lepidium sp.',  # assign to only other Lepidium
              'Microsteris sp.',#assign to only other Microsteris
              'Monolepis sp.', # assign to only other Monolepis
              'Munroa sp.',    # assign to only other Munroa
              'Myosurus sp.',  # assign to only other Myosurus
              'Oenothera sp.', # assign to only other Oenothera
              'Pediomelum sp.', #assign to only other Pediomelum
              'Vulpia sp.',    # assign these last two to the only Vulpia species
              'Vulpia octoflora hirtella')

#--! PROVIDE INFO !--#
good_name = c('Androsace occidentalis',
...
#--! PROVIDE INFO !--#                                 
  'A number of <Genus sp.> entries had only one member identified in the genus, and so were reassigned to that species, while several unknowns and unidentifieds with multiple possible identities were removed')
...
#--! PROVIDE INFO !--#
# quadrats are only 1 m2, and we seek to characterize the plant assemblage at
# this site as a whole, i.e. the set of 44 1-m2 quadrats.
